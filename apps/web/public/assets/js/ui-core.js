...
export function showEmptyState(containerId, title, message, icon = 'ðŸ“‚') {
  const container = document.getElementById(containerId);
  if (!container) return;
...
export function showErrorBoundary(containerId, error) {
  const container = document.getElementById(containerId);
  if (!container) return;
  container.innerHTML = '<div class="error-boundary" role="alert"><div class="error-icon" aria-hidden="true">âš ï¸/div><h3>Something went wrong</h3><p>' + (error.message || 'An unexpected error occurred.') + '</p><button onclick="window.location.reload()" class="btn btn-primary">Reload Page</button></div>';
}
